...
void ENReadnodeDemand(char *f_node_demand, struct TankStruct *tankcontrol, int timeperiod);
...
int NumValves=0; // No of valves that needs to be adjusted
...
float node_demande_inp [4][24] = {{5.324,5.324,5.324,5.324,10.648,26.620,53.240,66.550,66.550,53.240,26.620,26.620,21.296,21.296,26.620,15.972,21.296,26.620,39.930,39.930,26.620,21.296,15.972,10.648},{4.630,4.630,4.630,4.630,9.259,23.148,46.296,57.870,57.870,46.296,23.148,23.148,18.518,18.518,23.148,13.889,18.518,23.148,34.722,34.722,23.148,18.518,13.889,9.259},{1.157,1.157,1.157,1.157,2.315,5.787,11.574,14.468,14.468,11.574,5.787,5.787,4.630,4.630,5.787,3.472,4.630,5.787,8.681,8.681,5.787,4.630,3.472,2.315},{3.472,3.472,3.472,3.472,6.944,17.361,34.722,43.403,43.403,34.722,17.361,17.361,13.889,13.889,17.361,10.417,13.889,17.361,26.042,26.042,17.361,13.889,10.417,6.944}};
//float node_demande_inp [4][24] = {{5.324,5.324,5.324,5.324,10.648,26.620,53.240,66.550,66.550,53.240,26.620,26.620,21.296,21.296,26.620,15.972,21.296,26.620,39.930,39.930,26.620,21.296,15.972,10.648},{23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148,23.148},{1.157,1.157,1.157,1.157,2.315,5.787,11.574,14.468,14.468,11.574,5.787,5.787,4.630,4.630,5.787,3.472,4.630,5.787,8.681,8.681,5.787,4.630,3.472,2.315},{17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361,17.361}};
...
int main(int argc, char *argv[])
{
	...
	temp_valvestruct = (struct ValveStruct *)calloc(NumValves,sizeof(struct ValveStruct));
	...
	valvecontrol = (struct ValveStruct *) calloc(NumValves,sizeof(struct ValveStruct)); //allocate memory for valve structs.
	...
	temp_count2=0;
	char tempvalveID[MAX_VALVEID];	//Valve ID

	for(temp_count =0; temp_count < Nvalves; temp_count++) {

		ENgetlinkid(Valve[temp_count+1].Link,tempvalveID);
		if((strncmp(tempvalveID,"0944",4)!=0)&&(strncmp(tempvalveID,"0945",4)!=0)&&(strncmp(tempvalveID,"0946",4)!=0)&&(strncmp(tempvalveID,"1945",4)!=0)&&(strncmp(tempvalveID,"0001",4)!=0))
		{
			// Getting the link index
			valvecontrol[temp_count2].ValveLink = Valve[temp_count+1].Link;
			// Getting the Valve ID
			ENgetlinkid(Valve[temp_count+1].Link,valvecontrol[temp_count2].ValveID);
			if(DEBUG) {
				fprintf(fptr, "\n Valve Link of Valve %s is %d", valvecontrol[temp_count2].ValveID, valvecontrol[temp_count2].ValveLink);
				fflush(fptr);
			}

			valvecontrol[temp_count2].TimePeriod = timeperiod;
			fprintf(fptr, "\n Valve Id = %s \t ",valvecontrol[temp_count2].ValveID);
			fflush(fptr);

			// To get the Diameter of the valves for max flow calculation across a valve
			ENgetlinkvalue(valvecontrol[temp_count2].ValveLink,EN_DIAMETER, &(valvecontrol[temp_count2].Diameter));
			valvecontrol[temp_count2].MaxValue = Compute_MaxValue(valvecontrol[temp_count2].Diameter);
			fprintf(fptr, " Initial Valve Value = %f",valvecontrol[temp_count2].ValveValues[0]);
			fprintf(fptr, "\n Max Flow across Valve %s = %f \t Diameter is %f",valvecontrol[temp_count2].ValveID, valvecontrol[temp_count2].MaxValue, valvecontrol[temp_count2].Diameter);
			fflush(fptr);
			temp_count2++;
			//NumValves++;
		}
	...
	if(targetMode != TRUE)
	{
		for(temp_count=0;temp_count<Ntanks;temp_count++)
		{
			tankcontrol[temp_count].TankLevels[0]= 0.4 * tankcontrol[temp_count].MaxTankLevel; 
		}
	}

	int valveCount = 0;					// Initial Tank Count = 0;
	float avg_outflow = 0.0;
	//if(targetMode!=TRUE)
	//{
	while(valveCount < NumValves) 

	{
		if((strncmp(valvecontrol[valveCount].ValveID,"0944",4)==0)||(strncmp(valvecontrol[valveCount].ValveID,"0945",4)==0)||(strncmp(valvecontrol[valveCount].ValveID,"0946",4)==0)||(strncmp(valvecontrol[valveCount].ValveID,"0001",4)==0)||(strncmp(valvecontrol[valveCount].ValveID,"1945",4)==0))
		{
			valveCount++;
		}
		else if(strncmp(valvecontrol[valveCount].ValveID,"0933",4)==0)
		{
			if(!targetMode)
			{
				...
				for(;temp_count2<timeperiod;temp_count2++)
				{
					...
					valvecontrol[valveCount].ValveValues[temp_count2]= 45.834;
				}
			}
			else
			{
				for(temp_count=0;temp_count<Ntanks;temp_count++)
				{
					if(strncmp(tankcontrol[temp_count].TankID,"0916",4)==0)
					{		
						//	printf("\n Veeranageeri");
						avg_outflow = 0.0;
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"0934",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 26.62;
				}
			}
			else
			{	
				for(temp_count=0;temp_count<Ntanks;temp_count++)
				{
					if(strncmp(tankcontrol[temp_count].TankID,"917",3)==0)
					{		
						//	printf("\n NR_mohalla2");
						avg_outflow = 0.0;
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"0935",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 15.74;
				}
			}
			else
			{	
				for(temp_count=0;temp_count<Ntanks;temp_count++)
				{
					if(strncmp(tankcontrol[temp_count].TankID,"918",3)==0)
					{		
						//printf("\n NR_mohalla1");
						avg_outflow = 0.0;
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"0936",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 32.176;
				}
			}
			else
			{	
				for(temp_count=0;temp_count<Ntanks;temp_count++)
				{
					if(strncmp(tankcontrol[temp_count].TankID,"919",3)==0)
					{		
						//printf("\n Rajiv_nagar_I");
						avg_outflow = 0.0;
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
			...
			valveCount++;
				...
				else if(strncmp(valvecontrol[valveCount].ValveID,"1882",4)==0)
				{
					if(!targetMode)
					{
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
						for(;temp_count2<timeperiod;temp_count2++)
						{
							valvecontrol[valveCount].ValveValues[temp_count2]= 165.509;
						}
					}
					else
					{	
						for(temp_count=0;temp_count<Ntanks;temp_count++)
						{
							if((strncmp(tankcontrol[temp_count].TankID,"2343",4)==0)||(strncmp(tankcontrol[temp_count].TankID,"2351",4)==0)||(strncmp(tankcontrol[temp_count].TankID,"2335",4)==0)||(strncmp(tankcontrol[temp_count].TankID,"2327",4)==0)||(strncmp(tankcontrol[temp_count].TankID,"2261",4)==0))
							{		
								//printf("\n Rajendra_nagar2");
								avg_outflow = 0.0;
								temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"1942",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 236.75;
				}
			}
			else
			{	
				for(temp_count=0;temp_count<Ntanks;temp_count++)
				{
					if((strncmp(tankcontrol[temp_count].TankID,"2215",4)==0)||(strncmp(tankcontrol[temp_count].TankID,"2225",4)==0)||(strncmp(tankcontrol[temp_count].TankID,"2332",4)==0)||(strncmp(tankcontrol[temp_count].TankID,"2342",4)==0)||(strncmp(tankcontrol[temp_count].TankID,"2319",4)==0))
					{		
						//printf("\n Rajendra_nagar1");
						avg_outflow = 0.0;
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"2222",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 32.704;
				}
			}
			else
			{	
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"2229",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 34.837;
				}
			}
			else
			{	
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"2240",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 21.296;
				}
			}
			else
			{	
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"2247",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 23.726;
				}
			}
			else
			{	
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"2296",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 173.0;
				}
			}
			else
			{	
			...
			valveCount++;
				...
				else if(strncmp(valvecontrol[valveCount].ValveID,"2305",4)==0)
				{
					if(!targetMode)
					{
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
						for(;temp_count2<timeperiod;temp_count2++)
						{
							valvecontrol[valveCount].ValveValues[temp_count2]= 31.713;
						}
					}
					else
					{	
		...
		else if(strncmp(valvecontrol[valveCount].ValveID,"2322",4)==0)
		{
			if(!targetMode)
			{
				temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
				for(;temp_count2<timeperiod;temp_count2++)
				{
					valvecontrol[valveCount].ValveValues[temp_count2]= 34.722;
				}
			}
			else
			{	
			...
			valveCount++;
				...
				else if(strncmp(valvecontrol[valveCount].ValveID,"2330",4)==0)
				{
					if(!targetMode)
					{
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
						for(;temp_count2<timeperiod;temp_count2++)
						{
							valvecontrol[valveCount].ValveValues[temp_count2]= 23.958;
						}
					}
					else
					{	
				...
				else if(strncmp(valvecontrol[valveCount].ValveID,"2338",4)==0)
				{
					if(!targetMode)
					{
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
						for(;temp_count2<timeperiod;temp_count2++)
						{
							valvecontrol[valveCount].ValveValues[temp_count2]= 47.454;
						}
					}
					else
					{	
				...
				else if(strncmp(valvecontrol[valveCount].ValveID,"2346",4)==0)
				{
					if(!targetMode)
					{
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
						for(;temp_count2<timeperiod;temp_count2++)
						{
							valvecontrol[valveCount].ValveValues[temp_count2]= 31.481;
						}
					}
					else
					{	
				...
				else if(strncmp(valvecontrol[valveCount].ValveID,"2354",4)==0)
				{
					if(!targetMode)
					{
						temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
						for(;temp_count2<timeperiod;temp_count2++)
						{
							valvecontrol[valveCount].ValveValues[temp_count2]= 30.903;
						}
					}
					else
					{	
	...
	while(run_flag) {
		...
		fflush(fptr);
		printf("\n timeperiod = %d",timeperiod);
		printf("\n timeperiod 1 = %d",tankcontrol->TimePeriod);
		printf("\n Duration = %d",duration);
		fflush(stdout);
		//}
...
void ENReadValveSolutionFile(char *f_valve_level_input, struct TankStruct *tankcontrol)
{
	...
	while ((fgets(line, 1024, stream)!=NULL) && (valveCount < NumValves)) {
...
//	printf("\n NumValves %d",NumValves);
	valvecontrol_current = (struct ValveStruct *) calloc(NumValves,sizeof(struct ValveStruct));
	valvecontrol_previous = (struct ValveStruct *) calloc(NumValves,sizeof(struct ValveStruct));
	valvecontrol_gradient = (struct ValveStruct *) calloc(NumValves,sizeof(struct ValveStruct));
...
//	printf("\n NumValves %d",NumValves);
	memcpy(valvecontrol_current,valvecontrol,NumValves*sizeof(struct ValveStruct));
	memcpy(valvecontrol_previous,valvecontrol,NumValves*sizeof(struct ValveStruct));
	memcpy(valvecontrol_gradient,valvecontrol,NumValves*sizeof(struct ValveStruct));
...
//	memcpy(valvecontrol_gradient,valvecontrol,Nvalves*sizeof(struct ValveStruct));
	...
	//clk = clock()-clk;
	//printf("\n Time taken for func computation %f",((double)clk)*1000000/CLOCKS_PER_SEC);
	...
	while((iteration_count <= myOUTER_MAXITER)) {
		...
		memcpy(valvecontrol_previous,valvecontrol_current,NumValves*sizeof(struct ValveStruct));
...
//		memcpy(valvecontrol_previous,valvecontrol_current,Nvalves*sizeof(struct ValveStruct));
		...
		//clk = clock()-clk;
		//printf("\n Time taken for grad computation %f",((double)clk)/CLOCKS_PER_SEC);
		//clk = clock();
		update_control(tankcontrol_current, valvecontrol_current,tankcontrol_gradient,valvecontrol_gradient, iteration_count);
		//clk = clock()-clk;
		//printf("\n Time taken for updating controls computation %f",((double)clk)/CLOCKS_PER_SEC);
	...
	memcpy(valvecontrol,valvecontrol_current,NumValves*sizeof(struct ValveStruct));
...
//	printf("\n timeperiod = %d", timeperiod);
	...
	for(; temp_count <= timeperiod; temp_count++) {
		for(temp_count2 = 0; temp_count2 < Ntanks; temp_count2++) {
			if(strcmp(tankcontrol_current[temp_count2].TankID,"914")==0) continue;
			if(!targetMode)
			{
				...
				//	printf("\n Func1 = %s",tankcontrol_current[temp_count2].TankID);
	...
	//		if(strcmp(tankcontrol_current[temp_count2].TankID,"914")==0) continue;
		...
		for(temp_count2 = 0; temp_count2 < Ntanks; temp_count2 ++) {
	//		if(strcmp(tankcontrol_current[temp_count].TankID,"914")==0) continue;
	...
	for(temp_count = 1; temp_count < NumValves; temp_count++) {
//		if(!((strncmp(valvecontrol_current[temp_count].ValveID,"944",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"945",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"946",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"1",1)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"1945",4)==0)))
		{
			temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
...
void update_tank_level(struct TankStruct *tankcontrol)
{
	...
	for(; temp_count < timeperiod; temp_count++) {
		for(temp_count2 = 0; temp_count2 < Ntanks; temp_count2++) {
			if(strcmp(tankcontrol[temp_count2].TankID,"914")==0) continue;
...
void compute_sim(struct TankStruct *tankstruct, struct ValveStruct *valvestruct)
{
	...
	//for(temp_count = 0; temp_count< Nvalves; temp_count++) {
	for(temp_count = 0; temp_count< NumValves; temp_count++) {
		fprintf(valvePtr, "%s,", valvestruct[temp_count].ValveID);
		fflush(valvePtr);
		//if(temp_count == Nvalves-1) {
		if(temp_count == NumValves-1) {
			fprintf(valvePtr, "\n");
			fflush(valvePtr);
		}
	}
...
//	for(temp_count=0; temp_count<Nvalves; temp_count++) {
	for(temp_count=0; temp_count<NumValves; temp_count++) {
	//	if(!((strncmp(valvestruct[temp_count].ValveID,"944",3)==0)||(strncmp(valvestruct[temp_count].ValveID,"945",3)==0)||(strncmp(valvestruct[temp_count].ValveID,"946",3)==0)||(strncmp(valvestruct[temp_count].ValveID,"1",1)==0)||(strncmp(valvestruct[temp_count].ValveID,"1945",4)==0)))
		
			ENsetlinkvalue(valvestruct[temp_count].ValveLink,EN_INITSETTING,valvestruct[temp_count].ValveValues[simulation_time]);
	
	}
	int nodeindex[4];
	char *nodeID1 = "911",*nodeID3="1850",*nodeID2="1935", *nodeID4 = "1940";
	ENgetnodeindex(nodeID1,&nodeindex[0]);
	ENgetnodeindex(nodeID2,&nodeindex[1]);
	ENgetnodeindex(nodeID3,&nodeindex[2]);
	ENgetnodeindex(nodeID4,&nodeindex[3]);
//	for(temp_count=0;temp_count<4; temp_count++)
	for(temp_count=0;temp_count<4; temp_count++)
		{
			ENsetnodevalue(nodeindex[temp_count],EN_BASEDEMAND, node_demande_inp[temp_count][simulation_time]);
		}
		//	ENsetnodevalue(nodeindex[2],EN_BASEDEMAND, node_demande_inp[temp_count][6]);
		//	ENsetnodevalue(nodeindex[2],EN_BASEDEMAND, node_demande_inp[temp_count][6]);
...
//	temp_count = 0;
	...
	//printf("\n No of runs = %d",temp_count);

	int linkLoop; 
	float tempInFlow, tempOutFlow;
	for(temp_count=0; temp_count<Ntanks; temp_count++) {
	//	if(strcmp(tankstruct[temp_count].TankID,"914")==0) continue;
...
//	int rand_valve_vector[timeperiod*NumValves],rand_tank_vector[Ntanks];
	...
	//printf("\n Timeperiod = %d",timeperiod);
	rand_valve_vector = (int **)calloc(NumValves, sizeof(int *));	
//	rand_valve_vector = (int *)calloc(timeperiod*NumValves, sizeof(int));	
//	if(rand_valve_vector==NULL)
//		{
//			printf("Memory is not allocated");
//		}
//	printf("\n Size of rand_valve_Vec = %ld",sizeof(rand_valve_vector)/sizeof(int));
//	printf("\n Size of rand_valve_Vec = %ld",sizeof(rand_valve_vector));
	//rand_valve_vector = (int **)calloc(Nvalves, sizeof(int *));	
//	for(temp_count=0;temp_count<Nvalves;temp_count++)
	for(temp_count=0;temp_count<NumValves;temp_count++)
	{
		rand_valve_vector[temp_count]= (int*) calloc(timeperiod,sizeof(int));
		if(rand_valve_vector[temp_count]==NULL)
		{
			printf("\n Not enough memory 2089");
		}
	}
	rand_tank_vector = (int*) calloc(Ntanks,sizeof(int));
//	printf("\n Size of rand_tank_Vec = %ld",sizeof(rand_tank_vector)/sizeof(int));
	/*for(temp_count=0;temp_count<NumValves;temp_count++)
		{
			for(temp_count2=0;temp_count2<timeperiod;temp_count2++)
			{
				rand_valve_vector[temp_count][temp_count2]=0;
				printf("\n %d \t %d",temp_count,temp_count2);
			//	printf("\n %d",rand_valve_vector[temp_count][temp_count2]);
			}
		}
	*/
	//printf("\n Timeperiod = %d",timeperiod);
	memcpy(temp_tankstruct,tankcontrol_current,Ntanks*sizeof(struct TankStruct));
	memcpy(temp_valvestruct,valvecontrol_current,NumValves*sizeof(struct ValveStruct));
	//memcpy(temp_valvestruct,valvecontrol_current,Nvalves*sizeof(struct ValveStruct));

	//compute gradient for valves
	//for(temp_count =0; temp_count <Nvalves; temp_count++) {
	for(temp_count =0; temp_count <NumValves; temp_count++) {
//		if(!((strncmp(temp_valvestruct[temp_count].ValveID,"944",3)==0)||(strncmp(temp_valvestruct[temp_count].ValveID,"945",3)==0)||(strncmp(temp_valvestruct[temp_count].ValveID,"946",3)==0)||(strncmp(temp_valvestruct[temp_count].ValveID,"1",1)==0)||(strncmp(temp_valvestruct[temp_count].ValveID,"1945",4)==0)))
			temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime);
		//	printf("\n temp_count = %d",temp_count);
			for(; temp_count2 < timeperiod; temp_count2++) {
				temp_rand = (double) rand() / (RAND_MAX); 
	//			printf("\n temp_count = %d \t temp_count2 = %d",temp_count,temp_count2);
				if(temp_rand <=0.5)
				{
					rand_valve_vector[temp_count][temp_count2]=1; 
			//		rand_valve_vector[temp_count*NumValves+temp_count2]=1; 
	...
	if(!targetMode)
	{
		for(temp_count=0;temp_count<Ntanks;temp_count++)
		{
			if(strcmp(temp_tankstruct[temp_count].TankID,"914")==0) continue;
			temp_rand = (double) rand() / (RAND_MAX); 
			if(temp_rand<=0.5)
			{
...
//		if(!((strncmp(temp_valvestruct[temp_count].ValveID,"944",3)==0)||(strncmp(temp_valvestruct[temp_count].ValveID,"945",3)==0)||(strncmp(temp_valvestruct[temp_count].ValveID,"946",3)==0)||(strncmp(temp_valvestruct[temp_count].ValveID,"1",1)==0)||(strncmp(temp_valvestruct[temp_count].ValveID,"1945",4)==0)))
	...
	for(temp_count =0; temp_count <NumValves; temp_count++) {
		temp_count2 = ((targetMode == TRUE)? currentLevel.timeVal:startTime );
		for(; temp_count2 < timeperiod; temp_count2++) {
		//	if(!((strncmp(valvecontrol_gradient[temp_count].ValveID,"944",3)==0)||(strncmp(valvecontrol_gradient[temp_count].ValveID,"945",3)==0)||(strncmp(valvecontrol_gradient[temp_count].ValveID,"946",3)==0)||(strncmp(valvecontrol_gradient[temp_count].ValveID,"1",1)==0)||(strncmp(valvecontrol_gradient[temp_count].ValveID,"1945",4)==0)))
	//		{
...
void update_control(struct TankStruct *tankcontrol_current, struct ValveStruct *valvecontrol_current,struct TankStruct *tankcontrol_gradient,struct ValveStruct *valvecontrol_gradient, int main_iteration_count)
{
	...
	while(((function_value_next-function_value_current)>= -c *alpha_mult* norm_grad_var) && (iteration_count < myMAXITER)) {
		...
		for(; temp_count2<timeperiod; temp_count2++) {
			//for(temp_count =0; temp_count <Nvalves; temp_count++) {
			for(temp_count =0; temp_count <NumValves; temp_count++) {
//				if(!((strncmp(valvecontrol_current[temp_count].ValveID,"944",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"945",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"946",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"1",1)==0)))
	...
	for(; temp_count2<timeperiod; temp_count2++) {
		//for(temp_count =0; temp_count <Nvalves; temp_count++) {
		for(temp_count =0; temp_count <NumValves; temp_count++) {
//			if(!((strncmp(valvecontrol_current[temp_count].ValveID,"944",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"945",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"946",3)==0)||(strncmp(valvecontrol_current[temp_count].ValveID,"1",1)==0)||(strncmp(valvecontrol_gradient[temp_count].ValveID,"1945",4)==0)))
//			{
				temp_valve_value = valvecontrol_current[temp_count].ValveValues[temp_count2]-valvecontrol_gradient[temp_count].ValveValues[temp_count2]*alpha_mult;
...
//			}
	...
	memcpy(valvecontrol_current,temp_valvestruct,NumValves*sizeof(struct ValveStruct));
...
//		{
	...
	for(temp_count2=0; temp_count2<Ntanks; temp_count2++) {
		if(strcmp(tankcontrol_gradient[temp_count2].TankID,"914")==0) continue;
...
int Job_Handler(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol)
{
	...
	while(!Qempty()) {
		...
		if(!strncmp(p.keyword,"demand",6)) {
			...
			for(temp_count2 = 0; temp_count2 < NumValves; temp_count2++) {
				if(!strncmp(valvecontrol[temp_count2].ValveID,p.id,4)) {
...
void Job_Scheduler(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol)
{
	...
	float *avg = calloc(NumValves,sizeof(float));
	float valve_val = 0.0;
	//float *std_dev = calloc(Nvalves,sizeof(float));
	float *std_dev = calloc(NumValves,sizeof(float));
	...
	for(temp_count = 0; temp_count < NumValves; temp_count++) {
		...
		for(temp_count2 = 0; temp_count2 < valvecontrol[temp_count].TimePeriod; temp_count2 += hourly_scheduler) {
			...
			else if(fabs(valvecontrol[temp_count].ValveValues[temp_count2] - valve_val) >= 5) {
				// Pushing the actual Valve setting
				//printf("\n This is normal");
				//		printf("\n This is None");
				if(push_set_flag) {
