...
const int PRINT_INTERVAL_VALVEVALUES = 100;
...
int main(int argc, char *argv[])
{
	...
	if(targetMode == TRUE) {
		...
		//	printf("\n Solution tank level 0 %f",solutionTankLevel[0].tankId[loop]);
...
double objective_function(struct TankStruct *tankcontrol_current,struct ValveStruct *valvecontrol_current)
{
	...
	for(; temp_count < timeperiod+1; temp_count++) {
		for(temp_count2 = 0; temp_count2 < Ntanks; temp_count2++) {
			...
			func_value += 1.0 * w12 * pow(temp_float_var,2)/tankcontrol_current[temp_count2].MaxTankLevel;
		}
	}
	fflush(fptr);
	...
	if (!targetMode) {
		...
		for(temp_count2 = 0; temp_count2 < Ntanks; temp_count2 ++) {
//			printf("\n solution tank level 0 %f",solutionTankLevel[0].tankId[temp_count2]);
			temp_float_var = tankcontrol_current[temp_count2].TankLevels[timeperiod] - solutionTankLevel[0].tankId[temp_count2];
//			printf("\n tank control current %f",tankcontrol_current[0].TankLevels[timeperiod]);
//			temp_float_var = tankcontrol_current[temp_count2].TankLevels[timeperiod] - tankcontrol_current[temp_count2].TankLevels[timeperiod+1];
			...
			//@Ram made these changes
			func_value += 10*w3*duration * fabs(temp_float_var/tankcontrol_current[temp_count2].MaxTankLevel);
		//		func_value += w3 * duration * pow(temp_float_var,2)/tankcontrol_current[temp_count2].MaxTankLevel;
		}
	}
	...
	for(temp_count = 1; temp_count < Nvalves; temp_count++) {
		...
		for(; temp_count2 < timeperiod-1 ; temp_count2++) {
			...
			func_value += w4 *(2.0) * fabs(temp_float_var/valvecontrol_current[temp_count].MaxValue); 
		}
	}
...
//	printf("\n function value 1 = %f ", function_value_1);
...
//	printf("\n function value 2 = %f ", function_value_2);
	}
	/*	compute_flows(temp_tankstruct,temp_valvestruct,temp_count2);
		function_value_1 = objective_function(temp_tankstruct,temp_valvestruct);
	...
	*/
	...
	for(temp_count = 0; temp_count < Nvalves; temp_count++) {
		...
		for(temp_count2 = 0; temp_count2 < valvecontrol[temp_count].TimePeriod; temp_count2 += hourly_scheduler) {
			...
			//@Ram changed 20 to 2
			else if(fabs(valvecontrol[temp_count].ValveValues[temp_count2] - avg[temp_count]) <= 1) {
			...
			else if(fabs(valvecontrol[temp_count].ValveValues[temp_count2] - valve_val) >= 1) {
...
int feasiblity_checker(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol)
{
	...
	{
			printf("\n %d",temp_count);
			printf("\n %f \t %f",tankcontrol[temp_count].TankLevels[timeperiod],solutionTankLevel[0].tankId[temp_count]);
			printf("\t %f",(fabs(tankcontrol[temp_count].TankLevels[timeperiod]-solutionTankLevel[0].tankId[temp_count]))/tankcontrol[temp_count].MaxTankLevel);
	}
	for(temp_count = 0 ; temp_count < Ntanks; temp_count++)
	{
		...
		{
			//printf("\n Something else happens");
			if(fabs(tankcontrol[temp_count].TankLevels[0]- tankcontrol[temp_count].TankLevels[timeperiod])>(0.03*tankcontrol[temp_count].MaxTankLevel))	{
				//printf("\n Failure success");
		...
		{
			if(fabs(tankcontrol[temp_count].TankLevels[timeperiod]-solutionTankLevel[0].tankId[temp_count])>(0.012*tankcontrol[temp_count].MaxTankLevel))	{
			//if(fabs(tankcontrol[temp_count].TankLevels[timeperiod]-tankcontrol[temp_count].TankLevels[timeperiod+1])>(0.3*tankcontrol[temp_count].MaxTankLevel))	{
			//	printf("\n success");
				fprintf(fptr, "TankLevel[%d] :: StartTankLevel[%f] :: EndTankLevel[%f]\n", temp_count, tankcontrol[temp_count].TankLevels[0], tankcontrol[temp_count].TankLevels[timeperiod]);
				fprintf(fptr, " Tank Return Checking feasibility \n");
				fflush(fptr);
