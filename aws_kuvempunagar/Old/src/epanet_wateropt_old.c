...
float function_delta = 0.0001;  //Arbitary value taken
...
//			func_value += w3*duration * pow(temp_float_var,2)/tankcontrol_current[temp_count2].MaxTankLevel;
		...
		for(temp_count2 = 0; temp_count2 < Ntanks; temp_count2 ++) {
			...
			func_value += 500*w3 * duration * pow(temp_float_var,2)/tankcontrol_current[temp_count2].MaxTankLevel;
		}
	}
...
void Job_Scheduler(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol)
{
	...
	for(temp_count = 0; temp_count < Nvalves; temp_count++) {
		...
		for(temp_count2 = 0; temp_count2 < valvecontrol[temp_count].TimePeriod; temp_count2 += hourly_scheduler) {
			// Main pushing engine
			// @Ram changed the threshold from 10 to 5
			...
			//@Ram changed the threshold from 20 to 5
			else if(fabs(valvecontrol[temp_count].ValveValues[temp_count2] - avg[temp_count]) <= 2) {
			...
			else if(fabs(valvecontrol[temp_count].ValveValues[temp_count2] - valve_val) >= 2) {
...
int feasiblity_checker(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol)
{
	...
	for(temp_count = 0 ; temp_count < Ntanks; temp_count++)
	{
		...
		if(fabs(tankcontrol[temp_count].TankLevels[0]- tankcontrol[temp_count].TankLevels[timeperiod])>(.03*tankcontrol[temp_count].MaxTankLevel))	{
