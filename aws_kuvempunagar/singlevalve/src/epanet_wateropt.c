		...
		if(run_flag>0)
		{
			if(feasiblity_checker(tankcontrol, valvecontrol)) {
				...
				for(temp_count=0;temp_count<NumValves;temp_count++)
					{
						printf("\n");
						for(temp_count2=0;temp_count2<24;temp_count2++)
						{
						printf(" valvecontrol[%s][%d]= %f \n",valvecontrol[temp_count].ValveID,temp_count2,valvecontrol[temp_count].ValveValues[temp_count2]);
						}
					}
				...
				for(temp_count=0;temp_count<Ntanks;temp_count++)
					{
						...
						for(temp_count2=0;temp_count2<24;temp_count2++)
						{
						printf(" tankcontrol[%s][%d]= %f \n",tankcontrol[temp_count].TankID,temp_count2,tankcontrol[temp_count].TankLevels[temp_count2]);
						}
					}
		...
		void compute_gradient(struct TankStruct *tankcontrol_current, struct ValveStruct *valvecontrol_current, struct TankStruct *tankcontrol_gradient,struct ValveStruct *valvecontrol_gradient)
		{
			...
			for(temp_count =0; temp_count <NumValves; temp_count++) {
				...
				for(; temp_count2 < timeperiod; temp_count2++) {
					...
					{
						rand_valve_vector[temp_count][temp_count2]=rand_valve_vector[temp_count][0];
						temp_valvestruct[temp_count].ValveValues[temp_count2]=temp_valvestruct[temp_count].ValveValues[0];
					}
				}
							...
							void Job_Scheduler(struct TankStruct *tankcontrol, struct ValveStruct *valvecontrol)
							{
								...
								for(temp_count = 0; temp_count < NumValves; temp_count++) {
									...
									for(temp_count2 = 0; temp_count2 < valvecontrol[temp_count].TimePeriod; temp_count2 += hourly_scheduler) {
										...
										//else {
										if(std_dev[temp_count] <= 0.000001) {
											//			printf("\n This is std");
											//if(push_avg_flag) {
											//			printf("\n This is std");
										...
										else if(fabs(valvecontrol[temp_count].ValveValues[temp_count2] - valve_val) >= 0.000001*valvecontrol[temp_count].ValveValues[temp_count2]) {
											// Pushing the actual Valve setting
											//printf("\n This is normal");
											//		printf("\n This is None");
											if(push_set_flag) {
